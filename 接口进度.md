...
| 接口名称 | 状态 | 备注 | Bug |
|---------|--------|------------|---|
| [按条件查询拍卖会列表](首页/拍卖会信息相关接口.md) |开发完成| [测试](http://60.191.203.80/phones/pMainAction!getAuctionMainList.htm?status=预展中&type=同步)|1.关键字错误："stauts" 应改为 "status" 2.专场信息没有：dealNum, dealSum 3.status=已结束时返回为空 |
